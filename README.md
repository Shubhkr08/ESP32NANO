# ESP32NANO
I made a custom Esp32 board based on ESP32-S3-WROOM-1 , it has a USB type C and i tried to keep the size of the board as small as possible and to have as much as useful pins available on the board.
<br><img width="708" height="699" alt="Screenshot (42)" src="https://github.com/user-attachments/assets/dc092616-e929-439f-a4c6-099bc55b2817" />
<br>
## Overview
This project is a **compact 2-layer development board** based on the **ESP32-S3-WROOM-1 module**. It exposes **all usable GPIOs**, includes onboard **3.3V regulation**, and USB connectivity for easy programming and prototyping. 
The goal of this project is to **learn PCB design, microcontroller integration, and embedded programming**, while creating a versatile dev board for IoT, robotics, and electronics experiments.
## Features
- ESP32-S3-WROOM-1 module (Wi-Fi + Bluetooth 5.0)
- 2-layer compact PCB
- All GPIOs broken out (total 16+17= 33)
- USB connection for programming and power
- Onboard 3.3V voltage regulation
- Compact footprint for small projects
- Educational focus for learning MCU integration and PCB design
- ## Bill of Materials (BOM)
| Qty | Manufacturer Part Number | Description | Supplier | Unit Price (USD) | Total (USD) |
|----:|--------------------------|-------------|----------|------------------|-------------|
| 1 | ESP32-S3-WROOM-1-N16R8 | RF TXRX MODULE BT PCB TRACE SMD | DigiKey (digikey.in) | 6.56 | 6.56 |
| 1 | USB4125-GF-A | USB Type-C Receptacle 6-Pin SMD | DigiKey (digikey.in) | 0.57 | 0.57 |
| 5 | AMS1117-3.3 | 3.3V Linear Regulator | DigiKey (digikey.in) | 0.27 | 1.35 |
| 2 | PTS810SJG250SMTR LFS | Tactile Switch SPST-NO | DigiKey (digikey.in) | 0.57 | 1.14 |
| 1 | RS0805CT-1000GN-90TRD | 100Ω RF Resistor | DigiKey (digikey.in) | 2.90 | 2.90 |
| 1 | T529P106M010AAE200 | 10µF Tantalum Capacitor | DigiKey (digikey.in) | 1.26 | 1.26 |
| 1 | T529P226M010AAE200 | 22µF Tantalum Capacitor | DigiKey (digikey.in) | 1.32 | 1.32 |
| 1 | CL21B105KBFNNNE | 1µF Ceramic Capacitor | DigiKey (digikey.in) | 0.08 | 0.08 |
| 1 | CL21A226MAYNNNE | 22µF Ceramic Capacitor | DigiKey (digikey.in) | 0.08 | 0.08 |
| 2 | C0805C104K5RACTU | 0.1µF Ceramic Capacitor | DigiKey (digikey.in) | 0.08 | 0.16 |
| 2 | RC0805JR-075K1L | 5.1kΩ Resistor | DigiKey (digikey.in) | 0.10 | 0.20 |
| 1 | RC0805JR-0710KL | 10kΩ Resistor | DigiKey (digikey.in) | 0.10 | 0.10 |

### 🖨️ PCB & Shipping

- **DigiKey Shipping:** $20.00  
- **PCB + Stencil + Shipping:** $28.81 (JLCPCB)  
- **PCB Manufacturer:** JLCPCB — https://jlcpcb.com/

  
### 💰 Cost Summary

- **Components Total:** $15.72  
- **Shipping + PCB:** $48.81  
- **Estimated Total Cost:** **$64.53 USD**
## Schematic & PCB
<img width="1281" height="722" alt="Screenshot (39)" src="https://github.com/user-attachments/assets/4c8a113b-800f-4bb7-97dc-4ac0fb7dafd8" />
<img width="551" height="701" alt="Screenshot (53) (1)" src="https://github.com/user-attachments/assets/348d6fc1-adb0-4bc2-acf8-d68b8f36a3d0" />

- ## 3d Render<br><br>
<img width="770" height="902" alt="Screenshot (54)" src="https://github.com/user-attachments/assets/79eeb059-cd02-42a3-bdc6-99bb124d7e95" />
<img width="628" height="837" alt="Screenshot (55)" src="https://github.com/user-attachments/assets/9527e114-4158-4343-8ee0-eb73c8baa11d" />
<img width="899" height="851" alt="Screenshot (56)" src="https://github.com/user-attachments/assets/1b590b94-5381-4666-81f1-13276ff2e21a" />

